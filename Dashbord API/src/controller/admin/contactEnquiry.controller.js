...
exports.create =async (request, response) => {
    ...
    try {
        ...
        var saveData =await new contactModal(data).save()
            .then((result) => {
                const data = {
                    _status: true,
                    _message: 'Message sent succssfully',
                    _data: result
                }
                response.send(data);
            })
...
exports.view = async (request, response) => {
    ...
    try {
        ...
        total_record = await contactModal.find(filter).countDocuments()

        await contactModal.find(filter).select('name email mobile_number subject message status order').skip(skip).limit(limit).sort({ _id: 'desc' })
            .then((result) => {
                if (result.length > 0) {
...
exports.details = async (request, response) => {
    ...
    try {
        ...
        await contactModal.findById(request.params.id)
            .then((result) => {
                if (result) {
                    const data = {
                        _status: true,

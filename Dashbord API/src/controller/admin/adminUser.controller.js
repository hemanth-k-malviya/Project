...
exports.register = async (request, response) => {
    try {
        ...
        dataSave.role_type = 'super-admin'
        ...
        new userModal(dataSave).save()
            .then((result) => {
                ...
                const data = {
                    _status: true,
                    _message: "Admin Register succussfully!!",
                    _token: token,
                    _data: result
                }
                response.send(data)
...
exports.login = async (request, response) => {
    ...
    if (checkEmail.status == false) {
        const data = {
            _status: false,
            _message: 'Acoount is inactive. Please contact devloper !',
            _data: ''
        }
    }
...
exports.forgotPassword = async (request, response) => {
    ...
    try {
        ...
        var adminResetUrl = `${process.env.adminResetUrl}${token}`
        ...
        const htmlMessage = `
            <div style="font-family: Arial, sans-serif; line-height:1.6; color:#222;">
              ...
              <p><a href="${adminResetUrl}" style="background:#1a73e8;color:#ffffff;padding:10px 16px;border-radius:4px;text-decoration:none;display:inline-block;">Reset Password</a></p>

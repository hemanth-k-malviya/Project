...
export default function Newsletters() {
    ...
    useEffect(() => {
        axios.post(`${import.meta.env.VITE_BASE_URL}/${import.meta.env.VITE_NEWSLETTERS_API}/view`, {
            page: currentPage,
            name: searchName,
            email: searchEmail
        })
    ...
    const allCheckBoxSelect = () => {
        if (checkBoxValue.length == newsletter.length) {
            setcheckBoxValue([]);
            setButtonDisabled(true);
        } else {
            setcheckBoxValue([]);
    ...
    const changestatus = () => {
        axios.put(`${import.meta.env.VITE_BASE_URL}/${import.meta.env.VITE_NEWSLETTERS_API}/change-status`, {
            ids: checkBoxValue
        })
            .then((result) => {
                if (result.data._status == true) {
    ...
    const deleteRecords = () => {
        axios.put(`${import.meta.env.VITE_BASE_URL}/${import.meta.env.VITE_NEWSLETTERS_API}/delete`, {
            ids: checkBoxValue
        })
            .then((result) => {
                if (result.data._status == true) {
            ...
            <nav className="flex border-b-2" aria-label="Breadcrumb">
                <ol className="p-3 px-6 inline-flex items-center space-x-1 md:space-x-2 rtl:space-x-reverse">
                    <li className="inline-flex items-center ">
                        <Link to={"/dashbord"} className="inline-flex items-center text-md font-medium text-gray-700 hover:text-blue-600">
                            Home
                        </Link>
                    </li>
                    <li>
                        <div className="flex items-center">
                            /
                            <Link to={""} className="ms-1 text-md font-medium text-gray-700 hover:text-blue-600 md:ms-2">Newsletter</Link>
                        </div>
                    </li>
                                            ...
                                            <th scope="col" className="p-4">
                                                <div className="flex items-center">
                                                    <input
                                                        ...
                                                        checked={checkBoxValue.length == newsletter.length ? 'checked' : ''}
                                                        type="checkbox"
                                  ...
                                  <div className="rounded-[50%] w-[40px] h-[40px] flex items-center justify-center text-white bg-blue-700  border border-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">
                                    <MdModeEdit className='text-[18px]' />
                                                ...
                                                <tr class="bg-white  dark:bg-gray-800 dark:border-gray-700 border-gray-200 hover:bg-gray-50 dark:hover:bg-gray-600">
                                                    ...
                                                    <td colSpan={3} class=" px-6 py-4 text-gray-900 whitespace-nowrap dark:text-white">

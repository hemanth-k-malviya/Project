...
export default function ContactEnq() {
  ...
  useEffect(() => {
    axios.post(`${import.meta.env.VITE_BASE_URL}/${import.meta.env.VITE_CONTACT_US_API}/view`, {
      page: currentPage,
      name: searchName,
      email: searchEmail
    })
      .then((result) => {
        if (result.data._status === true) {
          setContactUs(result.data._data);
  ...
  const allCheckBoxSelect = () => {
    if (checkBoxValue.length == contactUs.length) {
      setcheckBoxValue([]);
      setButtonDisabled(true);
    } else {
      setcheckBoxValue([]);
  ...
  const changestatus = () => {
    axios.put(`${import.meta.env.VITE_BASE_URL}/${import.meta.env.VITE_CONTACT_US_API}/change-status`, {
      ids: checkBoxValue
    })
      .then((result) => {
        if (result.data._status == true) {
  ...
  const deleteRecords = () => {
    axios.put(`${import.meta.env.VITE_BASE_URL}/${import.meta.env.VITE_CONTACT_US_API}/delete`, {
      ids: checkBoxValue
    })
      .then((result) => {
        if (result.data._status == true) {
                      ...
                      <th scope="col" className="p-4">
                        <div className="flex items-center">
                          <input
                            ...
                            checked={checkBoxValue.length == contactUs.length ? 'checked' : ''}
                            type="checkbox"

...
export default function ViewTestimonials() {
  ...
  useEffect(() => {
    axios.post(`${import.meta.env.VITE_BASE_URL}/${import.meta.env.VITE_TESTIMONIALS_API}/view`, {
      page: currentPage,
      name: searchName
    })
      .then((result) => {
  ...
  const allCheckBoxSelect = () => {
    if (checkBoxValues.length == testimonial.length) {
      setCheckBoxValues([]);
      setButtonDisabled(true);
    } else {
      setCheckBoxValues([]);
  ...
  const changeStatus = () => {
    axios.post(`${import.meta.env.VITE_BASE_URL}/${import.meta.env.VITE_TESTIMONIALS_API}/change-status`, {
      ids: checkBoxValues
    })
      .then((result) => {
        if (result.data._status == true) {
  ...
  const deleteRecords = () => {
    axios.put(`${import.meta.env.VITE_BASE_URL}/${import.meta.env.VITE_TESTIMONIALS_API}/delete`, {
      ids: checkBoxValues
    })
      .then((result) => {
        if (result.data._status == true) {
  ...
  return (
    <section className="w-full">
      ...
      <Breadcrumb path={"testimonials"} link={'/view'} path2={"View"} slash={"/"} />
                      ...
                      <th scope="col" class="p-4">
                        <div class="flex items-center">
                          <input id="checkbox-all-search"
                            ...
                            checked={checkBoxValues.length == testimonial.length ? 'checked' : ''}
                          ...
                          return (
                            <tr class="bg-white  dark:bg-gray-800 dark:border-gray-700 border-gray-200 hover:bg-gray-50 dark:hover:bg-gray-600">
                              ...
                              <td class=" py-4">
                                ...
                                <Link to={`/testimonials/update/${v._id}`} >
